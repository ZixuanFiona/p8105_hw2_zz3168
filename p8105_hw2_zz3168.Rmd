...
According to the Baseline dataset, we could see that there have `r nrow(baseline_data)` participants were recruited. And based on the current age and age on set, we found `r mci_baseline_participant |> filter(current_age < age_at_onset) |> nrow()` develop MCI. And the average baseline age is `r baseline_data |> pull(current_age) |> mean() |> round(2)`years old. In the dataset, we found that `r baseline_data |> filter(sex == "female", apoe4 =="apoe4_carrier") |> nrow()/nrow(baseline_data) *100`% women in the study are APOE4 carriers.
